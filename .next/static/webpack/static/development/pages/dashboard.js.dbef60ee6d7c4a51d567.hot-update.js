  ...
  _createClass(_default, [{
        ...
        if (thisTradeHash != "undefined") {
          for (var j = 0; j < thisTradeHash.length; j++) {
            if (thisTradeHash[j] != "undefined") {
              var newArray = this.state.globalHash;
              newArray.push(thisTradeHash[j]);
              this.setState({
                globalHash: newArray
              });
            }
    ...
    value: function hashtagClicked(value) {
      ...
      if (this.state.globalHashActive[0] == undefined) {
        ...
        if (index > -1) {
          var _newArray = this.state.globalHash;

          _newArray.splice(index, 1);

          this.setState({
            globalHash: _newArray
          });
        }
      ...
      for (var i = 0; i < this.state.globalHashActive.length; i++) {
        if (this.state.globalHashActive[i] == "all") {
          ...
          if (_index > -1) {
            var _newArray3 = this.state.globalHash;

            _newArray3.splice(_index, 1);

            _newArray3.push("all");

            this.setState({
              globalHash: _newArray3
            });
          }

          var _newArray2 = this.state.globalHashActive;

          _newArray2.push(value);

          this.setState({
            globalHashActive: _newArray2
          });
          return;
        }
        ...
        if (i == this.state.globalHashActive.length - 1) {
          ...
          if (_index2 > -1) {
            var _newArray5 = this.state.globalHash;

            _newArray5.splice(_index2, 1);

            this.setState({
              globalHash: _newArray5
            });
          }

          var _newArray4 = this.state.globalHashActive;

          _newArray4.push(value);

          this.setState({
            globalHashActive: _newArray4
          });
          console.log(this.state.globalHashActive);
          console.log(this.state.globalHash);
          return;
...
//# sourceMappingURL=dashboard.js.dbef60ee6d7c4a51d567.hot-update.js.map