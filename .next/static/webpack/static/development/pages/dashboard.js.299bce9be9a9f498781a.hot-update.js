      ...
      for (var i = 0; i < data.fetchTradeHistory.length; i++) {
        ...
        if (i == data.fetchTradeHistory.length - 1) {
          for (var j = 0; j < fullRay.length; j++) {
            var dat = fullRay[j]; // let newDate = dat.timestamp;
            // let formattedDate = new Date(newDate);
            // dat.timestamp = formattedDate.toString();

            fullRay[j].timestamp = dat;
...
//# sourceMappingURL=dashboard.js.299bce9be9a9f498781a.hot-update.js.map